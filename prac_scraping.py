...
movie = db.naver_movie_list.find_one({'title':'록키'})
...
for p_num_1 in range(1,user_num_page+1):
    data = requests.get('https://movie.naver.com/movie/bi/mi/pointWriteFormList.nhn?code='+code_str+'&type=after&isActualPointWriteExecute=false&isMileageSubscriptionAlready=false&isMileageSubscriptionReject=false&page='+str(p_num_1),headers=headers)
    soup = BeautifulSoup(data.text, 'html.parser')
